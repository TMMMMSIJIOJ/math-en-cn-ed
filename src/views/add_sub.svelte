...
>
    <button
        class="bg-sky-500 border-none text-white px-4 py-1 cursor-pointer
            hover:bg-sky-400
            "
        on:click={submit}>生成(Generate)</button
    >
    <button
        class="bg-sky-500 border-none text-white px-4 py-1 cursor-pointer
            hover:bg-sky-400
        ...
        on:click={print}>打印(Print)</button
    >
    <span>
        <input
            id="showRes"
...
<span class="text-xs text-gray-400 mr-2">{index + 1}.</span>{item?.[1]
    ...
    {#if qrcodeStr}
        ...
        <div class=" justify-self-center self-center text-base">
            扫一扫 查答案 (Scan QR CODE For Answer)
        </div>
